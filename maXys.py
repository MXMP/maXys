...
def MT_Prepare_Data(macdata):
    ...
    :return:
    """
    # TODO: Все это нужно переработать, т.к. очень это нестабильно
    xlogger.debug("Начата обработка MAC-notify")
    mt_act = ''
    mt_mac = ''
    mt_port = ''
    # Преобразовываем символы в hex-string
    cutted_macdata = ByteToHex(macdata[-12:])
    xlogger.debug(cutted_macdata)
    # TODO: Дикий костыль для свичей DGS-1510-28L\ME и DGS-3000-28L
    action_flag = cutted_macdata[0] + cutted_macdata[1]
    if action_flag != '040A':
        xlogger.debug("Не удалось найти признак действия, отрезаем другой кусочек.")
        cutted_macdata = ByteToHex(macdata[-13:])
        action_flag = cutted_macdata[0] + cutted_macdata[1]
        xlogger.debug(cutted_macdata)
    ...
    try:
        xlogger.debug('Action flag: {}'.format(action_flag))
        mt_act = int(cutted_macdata[2], 10)  # - 'Действие' (или тип события). Бывает Add(1), Remove(2) и Move(3)
        mt_mac = ''.join(cutted_macdata[3:9])  # - MAC-адрес
        mt_port = int(cutted_macdata[10], 16)  # - Номер порта
    except:
        pass
    ...
    if (mt_act in [1, 2, 3]) & (len(mt_mac) == 12) & (mt_port != ''):
        xlogger.debug("MAC notify распознан:, {}, {}, {}".format(mt_mac, mt_act, mt_port))
        return mt_act, mt_mac, mt_port
    # Если же найдено что-то непонятное, возвращаем 'False' во всех случаях
    else:
        xlogger.debug("MAC notify не распознан.")
...
def SL_Prepare_Data(sysdata):
    ...
    if sysdata[0:2] == '<1':
        sl_data = sysdata[5:]
    if ('INFO:' in sl_data) & (sl_type == 0):
        sl_type = 1  # - Тип 'INFO'
    if ('WARN:' in sl_data) & (sl_type == 0):
        sl_type = 2  # - Тип 'WARN'
    if ('CRIT:' in sl_data) & (sl_type == 0):
        sl_type = 3  # - Тип 'CRIT'
...
def GetDevicesFromMySQL():
    ...
    try:
        mysql_db = MySQLdb.connect(host=mysql_addr, user=mysql_user, passwd=mysql_pass, db=mysql_base,
                                   connect_timeout=2)
...
def ApexSend(apexurl):
    ...
    try:
        ...
        if "INSERT_SUCCESS" in data:
            ...
            xlogger.info("Errrrrr: %s", data)
            return False
    except:
        xlogger.info("Errrrrr: %s, %s, %s", sys.exc_info()[0], sys.exc_info()[1], apexurl)
        return False
...
def main():
    ...
    if write_to_mysql:
        ...
        try:
            mysql_db_w = MySQLdb.connect(host=mysql_addr_w, user=mysql_user_w, passwd=mysql_pass_w, db=mysql_base_w,
                                         connect_timeout=1)
    ...
    while True:
        ...
        if int(time.time()) - timer >= interval:
            ...
            if m_chain > 1:
                ...
                if write_to_mysql & mysql_wready:
                    if PostDataToMySQL(mysql_cr_w, m_send_query[:-1] + ";"):
            ...
            if s_chain > 1:
                # Если включена запись в MySQL:
                if write_to_mysql & mysql_wready:
                    if PostDataToMySQL(mysql_cr_w, s_send_query[:-1] + ";"):
                        # Компенсируем автоматический инкремент счетчиков, отнимая единицу при подсчете
                        s_msql_cnt += 1 * s_chain - 1
                # Если включена запись в Oracle:
                if write_to_oracle:
                    if ApexSend(s_apexurl[:-22]):
                        # Компенсируем автоматический инкремент счетчиков, отнимая единицу при подсчете
            ...
            xlogger.info("Syslog messages: recieved %s, sended to MySQL %s, sended to Oracle %s", s_cnt, s_msql_cnt,
                         s_apex_cnt)
            xlogger.info("MAC Notification messages: recieved %s, sended to MySQL %s, sended to Oracle %s", m_cnt,
                         m_msql_cnt, m_apex_cnt)
            ...
            if write_to_mysql:
                ...
                # Пробуем подключиться к базе данных MySQL. Используем таймаут в 1 секунду
                try:
                    mysql_db_w = MySQLdb.connect(host=mysql_addr_w, user=mysql_user_w, passwd=mysql_pass_w,
                                                 db=mysql_base_w, connect_timeout=1)
                # Если возникла ошибка, сообщаем об этом в лог
                except:
                    xlogger.info("Cannot connect to MySQL. The data will not be stored. :(")
                    mysql_wready = False
        ...
        try:
            macdata, macaddr = mactrap.recvfrom(512)
        ...
        # Если ошибки не возникло, значит данные получены. Приступаем к их обработке
            ...
            if (mt_act is not False) & (mt_mac is not False) & (mt_port is not False):
                xlogger.debug("MAC notify распознан: \n\tAction: {}, \n\tMAC: {}, \n\tPort: {}".format(mt_act, mt_mac, mt_port))
                ...
                if (dev_id == 0) & (len(devices) > 0):
                    xlogger.info(
                        "MacTrap NOTIFY: Recieved MAC Notification-message from %s, but this device not found in database",
                        macaddr[0])
                # Если включена запись в log, формируем строку для лог-файла и пишем данные в лог
                if write_to_log:
                    mlogger.info(" %s%s%s  %s %s  %s", dev_id.rjust(5), dev_ip.rjust(12), mt_act, mt_mac,
                                 str(mt_port).rjust(3), int(time.time()))

                # Если цепочка пустая, формируем начало запроса
                if m_chain == 1:
                    m_send_query = "insert into {0}.{1} ({1}.switch_id,{1}.ip,{1}.action,{1}.mac,{1}.port,{1}.datetime) values ".format(
                        mysql_base_w, mysql_mtbl_w)
                    m_apexurl = apex_m_url + apex_m_query.encode("hex")
                # Достраиваем запрос для MySQL
                m_send_query += "('{0}','{1}','{2}','{3}','{4}','{5}'),".format(dev_id, dev_ip, mt_act, mt_mac, mt_port,
                                                                                int(time.time()))
                # Достраиваем ссылку для Oracle Apex
                m_apexurl += "SELECT {0},{1},{2},{3},'{4}',{5} FROM dual UNION ALL ".format(int(time.time()), dev_id,
                                                                                            dev_ip, mt_port, mt_mac,
                                                                                            mt_act).encode("hex")
        ...
        try:
            sysdata, sysaddr = syslog.recvfrom(512)
        # Если данных нет возникнет ошибка. Ничего не делаем
        except:
            pass
        # Если ошибки не возникло, значит данные получены. Приступаем к их обработке
            ...
            if len(sysdata) > 0:
                ...
                if (dev_id == 0) & (len(devices) > 0):
                    xlogger.info(
                        "SysLog  NOTIFY: Recieved SysLog-message from %s, but this device not found in database",
                        sysaddr[0])
                # Если включена запись в log, Формируем строку для лог-файла и пишем данные в лог
                if write_to_log:
                    slogger.info(" %s%s  %s  %s  %s", dev_id.rjust(5), dev_ip.rjust(12), int(time.time()), sl_type,
                                 sysdata)

                # Если цепочка пустая, формируем запрос для записи в базу данных и помещаем в него данные:
                if s_chain == 1:
                    s_send_query = (
                        "insert into {0}.{1} ({1}.switch_id,{1}.ip,{1}.type,{1}.data,{1}.datetime) values ").format(
                        mysql_base_w, mysql_stbl_w)
                    s_apexurl = apex_s_url + apex_s_query.encode("hex")
                # Достраиваем запрос для MySQL
                s_send_query += "('{0}','{1}','{2}',SUBSTR('{3}',1,160),'{4}'),".format(dev_id, dev_ip, sl_type,
                                                                                        sysdata, int(time.time()))
                # Достраиваем ссылку для Oracle Apex
                s_apexurl += "SELECT {0},{1},{2},{3},'{4}' FROM dual UNION ALL ".format(int(time.time()), dev_id,
                                                                                        dev_ip, sl_type,
                                                                                        sysdata).encode("hex")
